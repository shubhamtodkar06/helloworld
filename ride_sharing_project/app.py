...
def success():
     ...
     person = accept.execute('SELECT accepter FROM accept')
     users = list()
     persons = list()
     ...
     for values in person:
          persons.append(values['accepter'])
     if userid in users :
          if userid in persons :
               return render_template("successful.html", name = session.get("name").upper(), button = 'Check', button2 = 'Details')
          else:
               return render_template("successful.html", name = session.get("name").upper(), button = 'Check', button2 = 'Search')
     else:
          if userid in persons :
               return render_template("successful.html", name = session.get("name").upper(), button = 'Create', button2 = 'Details')
          else:
               return render_template("successful.html", name = session.get("name").upper(), button = 'Create', button2 = 'Search')
...
def Search():
     ...
     if request.method == 'GET':
          ...
          for i in range(number):
               print(i)
               if not pick.split()[i] and not drop_l.split()[i]:
                    return render_template('noride.html')
               ...
               time1 = request.form.get('time1')
               if act == 'captain':
                    info = rides.execute('SELECT * FROM rides WHERE pick LIKE ? AND drop_l LIKE ? AND who = ? AND passengers > ?', pick_new, drop_l_new , 'cruiser', pass_num)
               else:
                    info = rides.execute('SELECT * FROM rides WHERE pick LIKE ? AND drop_l LIKE ? AND who = ? AND passengers > ?', pick_new, drop_l_new , 'captain', pass_num)
               print(info)
               if info:
                    break

          return render_template('show.html', info = info, pass_num = pass_num)
...
def delete():
     ...
     if request.method == 'GET':
          ...
          accept.execute('DELETE FROM accept WHERE creater = ?', session.get('user_id'))
          return redirect('/success')
...
def show():
     ...
     if request.method == 'POST':
          ...
          print(creator)
          print(acceptor)
          accept.execute('INSERT INTO accept VALUES (?,?)', acceptor, creator)
          rides.execute('UPDATE rides SET passengers = passengers - 1 WHERE userid = ?', creator)
...
def details():
     ...
     if request.method == 'GET':
          ...
          userid = userid[0]
          userid = userid['creater']
          print('heellloo')
          print(userid)
          info = db.execute('SELECT * FROM loger WHERE user_id = ?', userid)
          ride_info = rides.execute('SELECT * FROM rides WHERE userid = ?', userid)
          print(info)
          print(ride_info)
          return render_template('details.html', info = info, ride_info = ride_info)
     else:
          return redirect('/success')

#########################################################################################################################################

@app.route("/cancel_ride", methods=["GET","POST"])
def cancel():
     if not session.get("name"):
          return redirect("/login")
     if request.method == 'POST':
          userid = session.get('user_id')
          accept.execute('DELETE FROM accept WHERE accepter = ?', userid)
          return redirect('/success')
